...
module.exports = function(cfg) {
  ...
  var updateTableField = function(base, reqBody, callback) {
    var data = {
  ...
  var updateContentField = function(base, reqBody, callback) {
    var data = reqBody;
    data.content = (data.content == "true") ? true : data.content;
    data.content = (data.content == "false") ? false : data.content;

    updateMetaData("editor", base, reqBody, callback);  
  };
  ...
  var contentForSelect = function(files) {
      if (!files) return [];
      var optFiles = [], i;

      optFiles.push({value:false, name:'No Content Window'}); 
      optFiles.push({value:true, name:'Include Content Window'}); 

      for (i = 0; i < files.length; i++) {
        optFiles.push({value: utils.normaizeFilePartExt(files[i]), name: 'Clone from ' + utils.normaizeFilePartExt(files[i]) })
      }
  ...
  return {
    ...
    contentForSelect:   contentForSelect
  };
};